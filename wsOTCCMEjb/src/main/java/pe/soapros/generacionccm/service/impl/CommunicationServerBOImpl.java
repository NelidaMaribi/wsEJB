...
public class CommunicationServerBOImpl implements CommunicationServerBO {
	...
	private String executeFlujoUnico(String urlFlujo, String name, String version, String method, String solicitud, String key, String async) throws Exception {
		...
		try {
			...
			try {
			plantilla = jsonInput.get("cabecera").get("detalleTXT").get("codigoPlantilla").asText();
			}catch(Exception e1) {
		...
		if (conn.getResponseCode() != 200) {
			logger.error("Error al consultar servicio: " + urlFlujo + " - [HTTP ERROR CODE]: " + conn.getResponseCode());
			throw new Exception(
	...
	private String executeToken(String urlToken, String usuario, String password, String method) throws Exception {
		...
		logger.debug("Response: " + conn.getResponseCode());
		if (conn.getResponseCode() != 200) {
			logger.error("Error al consultar servicio: " + urlToken + " - [HTTP ERROR CODE]: " + conn.getResponseCode());
			throw new Exception(
